...
if [ -z "$Ed25519Key" ]; then # Checks if key exists
	...
	if [ "$exists" == "true" ]; then
		...
		sig=$(tide-key sign $Ed25519Key $url)
		echo $url
		bash -c "sleep 7; curl -s --location --request POST 'https://h4x22simulator.azurewebsites.net/orks/update' --form 'orkName="$1"' --form 'newOrkUrl="$url"' --form 'signedOrkUrl="$sig"';" & 
